...
export async function POST(request: NextRequest) {
  try {
    ...
    const maxFileSize = isPro ? 200 * 1024 * 1024 : 50 * 1024 * 1024 // 200MB vs 50MB

    if (files.length > maxFiles) {
      return NextResponse.json({ 
        error: `무료 버전은 최대 ${maxFiles}개 파일만 처리 가능합니다. Pro 업그레이드를 고려해보세요.`,
        requiresPro: true 
      }, { status: 403 })
    }
    ...
    for (const file of files) {
      if (file.size > maxFileSize) {
        return NextResponse.json({ 
          error: `파일 크기가 ${isPro ? '200MB' : '50MB'}를 초과합니다: ${file.name}. Pro 업그레이드를 고려해보세요.`,
          requiresPro: true 
        }, { status: 413 })
      }
    }
